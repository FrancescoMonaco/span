...
namespace puffinn{
    ...
    class EMST {
        ...
        std::vector<std::vector<EdgeTuple>> Tu;
        std::vector<std::vector<EdgeTuple>> Tc;
        std::vector<std::vector<std::vector<unsigned int>>> neighbors;
        std::vector<std::vector<unsigned int>> global_neighbors;

        std::vector<EdgeTuple> top;
        std::vector<EdgeTuple> global_Tu;
        std::vector<std::vector<EdgeTuple>> local_top;
        ...
        public:
                  ...
                  dsu_true(DSU(num_data))
            {
                ...
                neighbors.resize(MAX_REPETITIONS);
                for (auto& local : neighbors) {
                    local.resize(num_data);
                }
                global_neighbors.resize(num_data);
                local_top.resize(MAX_REPETITIONS);
                Tc.resize(MAX_REPETITIONS);
                Tu.resize(MAX_REPETITIONS);
                //dirty_start();
            ...
            float exact_tree(){
                ...
                for (const auto& edge : all_edges) {
                    add_edge(edge, dsu, top, global_neighbors);
                }
            ...
            std::vector<std::pair<unsigned int, unsigned int>> find_epsilon_tree() {
                ...
                std::vector<std::pair<unsigned int, unsigned int>> global_tree;
                std::vector<EdgeTuple> global_top;
                std::vector<size_t> finished;
                bool found = false;

                #pragma omp parallel shared(found, finished, global_tree, global_top)
                {
                ...
                for (int i=MAX_HASHBITS; i>= 0; i--) {
                   ...
                   // std::cout << "Iteration: " << segments[0].i <<" "<< i << std::endl;
                    for (size_t j=0; j<MAX_REPETITIONS; j++) {
                        ...
                        std::vector<EdgeTuple> local_Tu, local_Tc;
                        enumerate_edges(segments[j], local_Tu, local_Tc);
                        std::cout << "Tu_l size: " << local_Tu.size() << " Tc_l size: " << local_Tc.size() << std::endl;

                        for(auto edge : local_Tc) {
                            auto first_edge = std::get<1>(edge).first;
                            auto second_edge = std::get<1>(edge).second;
                            std::cout << "We here" << std::endl;
                            if (std::find(neighbors[j][first_edge].begin(), neighbors[j][first_edge].end(), second_edge) != neighbors[j][first_edge].end())
                                continue;
                            if(add_edge(edge, local_dsu, local_top[j], neighbors[j])) {
                                Tc[j].push_back(edge);
                            }
                        ...
                        for(auto edge : edges_to_move) {
                            Tu[j].erase(std::remove(Tu[j].begin(), Tu[j].end(), edge), Tu[j].end());
                        }

                        construct_tree(local_dsu, local_top[j], Tc[j], Tu[j], j);
                        std::cout << "Local top size: " << local_top[j].size() << std::endl;

                        #pragma omp critical
                        finished.push_back(j);
                        std::cout << "Finished size: " << finished.size() << std::endl;
                    }
                ...
                #pragma omp single
                {
                //Fill the tree, work for the master thread
                while (!found && !finished.empty()) {
                    std::cout << "Master thread" << std::endl;

                    size_t j = finished.back();
                    finished.pop_back();
                    if (top.size() == num_data - 1) {
                        std::tie(global_tree, found) = fill_tree(dsu_true, local_top[j], global_top);
                        if (found) {
                            is_connected(global_tree);
                        }
                    }
                }
                }
        ...
        private:
            ...
            void construct_tree(DSU &dsu, std::vector<EdgeTuple>& top, std::vector<EdgeTuple>& Tc, std::vector<EdgeTuple>& Tu, int j) {
                for (const auto& edge : Tc) {
                    add_edge(edge, dsu, top, neighbors[j]);
                    if (top.size() == num_data - 1) {
                        return;
                    }
                }
            ...
            bool add_edge(const EdgeTuple& new_edge_input, DSU &dsu, std::vector<EdgeTuple>& edge_list, std::vector<std::vector<unsigned int>>& neighbors) {
                ...
                dfs_cycle_discover(new_edge.first, new_edge.second, cycleEdges, neighbors);
                if (cycleEdges.empty()) {
            ...
            std::pair<std::vector<std::pair<unsigned int, unsigned int>>, bool> fill_tree(DSU& dsu_copy, std::vector<EdgeTuple>& Tc, std::vector<EdgeTuple>& Tu) {
                std::vector<std::pair<unsigned int, unsigned int>> tree;
                ...
                for (const auto& edge : Tc) {
                    ...
                    add_edge(edge, dsu_copy, top_copy, neighbors[0]);
                }

                std::sort (global_Tu.begin(), global_Tu.end());
                for (const auto& edge : global_Tu) {
                    // Finish the tree with the best unconfirmed edges
                    if (top_copy.size() == num_data - 1) {
                        break;
                    }
            ...
            void dirty_start() {
                ...
                for (size_t i = 1; i < vertices.size(); i++) {
                    global_Tu.emplace_back(1-table.get_similarity(vertices[i-1], vertices[i]), std::make_pair(vertices[i-1], vertices[i]));
            ...
            void dfs_cycle_discover(unsigned int source, unsigned int find, std::vector<EdgeTuple> &cycleEdges, std::vector<std::vector<unsigned int>>& neighbors) {
               // return;
