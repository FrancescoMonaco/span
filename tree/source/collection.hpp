...
namespace puffinn {
    ...
    public:
        ...
        // Retrieve the number of hash values used internally.
        size_t get_hashbits() const {
            return MAX_HASHBITS;
        }
        ...
        void merge_segments(std::vector<CollisionEnumerator>& segments) {
            ...
            for (auto& segment : segments) {
                ...
                for (size_t f = 0; f < segment.ranges.size(); f++) {
                    ...
                    new_range.insert(new_range.end(), std::make_pair(range_curr.begin()->first, (std::prev(range_curr.end()))->second));
                    for (int g = 1; g <= offset; g++) {
                        auto range_g = segment.ranges[f + g];
                        new_range.insert(new_range.end(), std::make_pair(range_g.begin()->first, (std::prev(range_g.end()))->second));
                    }
                    //Print the new range
                    // std::cout << "New range \n";
        ...
        /// Find the top-k closest pairs, with the given recall
        std::vector<std::pair<unsigned int, unsigned int>> closest_pairs(
            unsigned int k,
            float recall,
            FilterType /*filter_type*/ = FilterType::Default
        ) {
